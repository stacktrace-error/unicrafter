...
public class Recipe extends UnlockableContent{
    ...
    /**Sets the requirements at once, instead of needing you to type 5 fields manually.
    ...
    /**Sets the outputs at once, instead of needing you to type 5 fields manually.
     * @return the array it performed the checks on, for super chaining.*/
    ...
    public void craft(UniversalBuild build){
        ...
        if(itemOut().size != 0) itemOut.each(output -> {
            for(int i = 0; i < output.amount; i++){
                build.offload(output.item);
            }
        });

        if(liqOut().size != 0) liqOut.each(output -> {
            for(int i = 0; i < output.amount; i++){
    ...
    public boolean shouldConsume(UniversalBuild build){
        if(itemOut().size != 0){
            for(int i = 0; i < itemOut().size; i++){
                if(build.items.get(itemOut().get(i).item) + itemOut().get(i).amount > build.block.itemCapacity){
                    return false;
                }
            }
        }
        ...
        if(liqOut().size != 0 && !ignoreLiquidFullness){
            ...
            for(int i = 0; i < liqOut().size; i++){
                if(build.liquids.get(liqOut().get(i).liquid) + liqOut().get(i).amount > build.block.liquidCapacity){
                    if(!dumpExtraLiquid){
                        return false;
                    }
                }else{
